...
class BetaBernoulliArm:
    def __init__(self, alpha: float, k: int, prior_a: float = 1.0, prior_b: float = 1.0):
        self.alpha = alpha 
        self.k = k
        self.a = prior_a      # α of Beta distribution 
        self.b = prior_b      # β of Beta distribution 

    def sample(self) -> float:
        # Sample from Beta(a, b)
        return random.betavariate(self.a, self.b)

    def update(self, reward: float, threshold: float = 0.5):
        # Y=1 if reward>=threshold else 0
        y = 1 if reward >= threshold else 0
        self.a += y
        self.b += (1 - y)
...
class ThompsonSamplingBandit:
    def __init__(self, alphas: List[float], ks: List[int], prior_a: float = 1.0, prior_b: float = 1.0):
        self.arms: List[BetaBernoulliArm] = [BetaBernoulliArm(a, k, prior_a, prior_b)
                                            for a in alphas for k in ks]

    def select_arm(self) -> BetaBernoulliArm:
...
def parse_scores(text: str) -> Tuple[float, float]:
    """
    Extract Novelty and Feasibility
    """
    def _find(label: str, s: str) -> float:
        # match pattern for label
        pattern = rf"[*\- ]*{label}[\s\*]*(?:Rating)?[\s\*]*[:=]?\s*([0-9]+(?:\.[0-9]+)?)(?:/10)?"
        m = re.search(pattern, s, flags=re.I)
        if m:
            return float(m.group(1))
        return 0.0

    novelty     = _find("novelty", text)
    feasibility = _find("feasibility", text)
    return novelty, feasibility


def compute_path_reward(path: str) -> float:
    """
    rewards = lam * novelty + (1-lam) * feasibility
    """
    from ScienceDiscovery.agents import rate_novelty_feasibility
    res_txt = rate_novelty_feasibility(path)
    nov, feas = parse_scores(res_txt)

    lam = 0.5
    return lam * (nov / 10.0) + (1 - lam) * (feas / 10.0)
...
def optimize_parameters(
    ...
    """
    Beta-Bernoulli Thompson Sampling to search (alpha, k)。
    """
    from ScienceDiscovery.agents import generate_path
    ...
    for _ in range(trials):
        ...
        path = generate_path(arm.alpha, arm.k)
        reward = compute_path_reward(path)
        arm.update(reward)

        if reward > best_score:
            best_score, best_arm = reward, arm

    return best_arm.alpha, best_arm.k
# ---------------------------------------------------------------------------
